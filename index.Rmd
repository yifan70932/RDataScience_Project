...
The column of ISO code for each country is first stored as character variable and the population column is stored seperately as numeric variable. Then these two sets of values are joined together in to a data frame. 

The joinCountryData2Map function turns the data frame into a spatial polygon data frame by specify the "joincode", which is the ISO2 code in our scenario.
...
In this part, a simple choropleth visualization is performed. To save space, only the religious population in the year 2010 and 2050 is plotted in the choropleth to show a general trend of the country-scale distribution and the changing of religious population. 

The map is produced via "rworldmap" package and the color is chosed from "RColorBrewer" package.

The mapCountryData method is called to plot the spatial polygon data frame created. The legend of the map is created seperately afterwards using addMapLegend function to make sure the numbers and letters could be controled.
...
This page, the pie chart representing the population proportion of different religion in each country from 2010 to 2050 is produced.

The polulation value in spatial polygon data frames for every religion in each year is retrieved and combined into a new spatial data frame. Then plotted via mapPies function provided by the rworldmap package
...
Bubble Plot for the 2010 Muslim Population
```{r, message=FALSE, warning=FALSE}
mapBubbles(islam_2010_plot, 
           nameZSize = 'value', 
           catMethod = "logFixedWidth",
           numCats = 5,
           main = 'Muslim Population 2010',
           landCol = 'wheat',
           oceanCol = 'light blue')
```
